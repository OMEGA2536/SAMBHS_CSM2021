...
namespace SAMBHS.Windows.WinClient.UI.Procesos
{
    public partial class frmRegistroVentaRapida : Form
    {
        ...
        public frmRegistroVentaRapida(string Modo, string IdVenta, List<string> servicios, string cotizacionId = "")
        {
        ...
        private void frmRegistroVenta_Load(object sender, EventArgs e)
        {
            ...
            var RoleName = Globals.ClientSession.v_RoleName;
            ...
            using (new LoadingClass.PleaseWait(this.Location, "Por favor espere..."))
            {
                ...
                else if (RoleName == "CAJA OCUPACIONAL")
                {
                    cboTipoServicio.Value = 1;
                }
                else if (RoleName == "FARMACIA")
                {
            ...
            if (strModo == "Agenda")
            {
               txtRucCliente.Text = ObtenerDNI(listaServicios[0]);
               {
        ...
        private string ObtenerDNI(string p)
        {
            ConexionSigesoft conectasam = new ConexionSigesoft();
            conectasam.opensigesoft();
            string fecha = DateTime.Now.ToShortDateString();
            var cadena = "select PP.v_DocNumber from service SR inner join person PP on SR.v_PersonId=PP.v_PersonId where SR.v_ServiceId='"+p+"'";
            var comando = new SqlCommand(cadena, connection: conectasam.conectarsigesoft);
            var lector = comando.ExecuteReader();
            string dni = "";
            while (lector.Read())
            {
                dni = lector.GetValue(0).ToString();
            }
            lector.Close();
        ...
        public void txtRucCliente_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            //{
                ...
                if (ValidaCamposNulosVacios())
                {
                    foreach (UltraGridRow Fila in grdData.Rows)
                    {
                        ...
                        //valida cero para recibo interno
                        if (cboDocumento.Text != "RSM")
                        {
                ...
                if (objOperationResult.Success == 1)
                {
                    ...
                    if (cboCondicionPago.Text == "PAGO MIXTO")
                    {
                        frmCobranzaRapida frm = new frmCobranzaRapida(strIdVenta,0,0);
                        frm.ShowDialog();
                    }
                    ...
                    //new FacturaloPeruManager().EnviarFactura("N001-ZQ000014155");
                    //var dataResp = new FacturaloPeruManager().EnviarFactura(_ventaDto.v_IdVenta);
                    //if (dataResp.success)
                    //{
                    //    MessageBox.Show(dataResp.response.description, "HECHO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    //    Process.Start(dataResp.links.pdf + "/ticket");
                    //}
                    //else
                    //{
                    //    MessageBox.Show("Sucedió un error al hacer el envío a la SUNAT, por favor reenvielo desde la bandeja de registros de ventas. Error: " + dataResp.message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //    return;
                    //}
            ...
            //    UltraMessageBox.Show("Por favor llene los campos requeridos antes de guardar", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                ...
                if (listaServicios.Count !=0 )
                {
                    ...
                    {
                        var cadena1 = "update service set i_IsFac = 2, v_ComprobantePago = '" + txtSerieDoc.Text + "-" + txtCorrelativoDocIni.Text + "' where v_ServiceId='" + item + "'";

                        SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
                        SqlDataReader lector = comando.ExecuteReader();
                        lector.Close();

                        var qry = "update servicecomponent set i_IsFact = 1  where v_ServiceId = '" + item + "'";

                        SqlCommand comando2 = new SqlCommand(qry, connection: conectasam.conectarsigesoft);
                ...
                if (strModo == "Cotizacion")
                {
                    var objCotizacion = CotizacionBL.GetDataCotizacionById(_cotizacionId);

                    var total = txtTotal.Text;
                    objCotizacion.d_aCuenta += decimal.Parse(total);
                    objCotizacion.d_Saldo = objCotizacion.d_CostoTotal - objCotizacion.d_aCuenta;
                    var OK = CotizacionBL.UpdateCotizacion(objCotizacion, Globals.ClientSession.i_SystemUserId);
                    if (!OK)
                    {
                        MessageBox.Show("Sucedió un error al actualizar la cotización, por favor vuelva a intentar.",
        ...
        private void txtMes_Leave(object sender, EventArgs e)
        {
            ...
            if (txtMes.Text != "")
            {
                ...
                if (Mes >= 1 && Mes <= 12)
                {
                    if (strModo == "Nuevo" || strModo == "Cotizacion" || strModo == "Agenda")
                    {
                        ObtenerListadoVentas(txtPeriodo.Text, txtMes.Text);
                    }
        ...
        private void txtMes_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (txtMes.Text != "")
                {
                    ...
                    if (Mes >= 1 && Mes <= 12)
                    {
                        if (strModo == "Nuevo" || strModo == "Cotizacion" || strModo == "Agenda" || strModo == "Agenda")
                        {
                            ObtenerListadoVentas(txtPeriodo.Text, txtMes.Text);
                        }
                        else if (strModo == "Guardado")
        ...
        private void BtnImprimir_Click(object sender, EventArgs e)
        {
            ...
            if (idDoc == (int)DocumentType.EGRESO_CAJA_OCUPACIONAL)
            {
                ...
                using (new LoadingClass.PleaseWait(this.Location, "Generando..."))
                {
                    ...
                    if (datosP.Count() >= 1)
                    {
                        ...
                        string nombre = txtSerieDoc.Text + "-" + txtCorrelativoDocIni.Text + " - CSM";
            ...
            else if (idDoc == (int)DocumentType.RECIBO_SAN_LORENZO)
            {
                ...
                string nroRecibo = txtSerieDoc.Text + "-" + txtCorrelativo.Text;
                if (datosP.Count() >= 1)
                {
                    ...
                    Recibo_Interno.CreateRecibo(ruta, DatosPaciente, calendar, fechaNacimiento, serie_correlativo, datosP);
        ...
        private void cboDocumento_Leave(object sender, EventArgs e)
        {
            if (cboDocumento.Value != null)
            {
                if (strModo == "Nuevo" || strModo == "Cotizacion" || strModo == "Agenda")
                {
                    ...
                    if (cboDocumento.Value.ToString() != "7" && cboDocumento.Value.ToString() != "1" && cboDocumento.Value.ToString() != "8" && strModo != "Agenda")
                    {
                        ClientePublicoGeneral();
                    }
            ...
            switch (e.Cell.Column.Key)
            {
                case "v_CodigoInterno":
                    ...
                    if (grdData.ActiveRow.Cells["i_RegistroTipo"].Value.ToString() == "Temporal")
                    {
                        ...
                        {
                            productoshortDto prod = Globals.ClientSession.i_UsaListaPrecios == 0
                            ...
                            if (prod != null)
                            {
                                ...
                                grdData.ActiveRow.Cells["v_NroCuenta"].Value = "101011";//prod.NroCuentaVenta;
                        ...
                        {
                            frm = new Mantenimientos.frmBuscarProducto(int.Parse(grdData.ActiveRow.Cells["i_IdAlmacen"].Value.ToString()), "PedidoVenta", _ventaDto.v_IdCliente, grdData.ActiveRow.Cells[e.Cell.Column.Key].Text ?? string.Empty, UserConfig.Default.appTipoBusquedaPredeterminadaProducto);
                            frm.ShowDialog();
                            if (frm._NombreProducto != null)
                            {
                                ...
                                grdData.ActiveRow.Cells["v_NroCuenta"].Value = "101011";
        ...
        {
            if (!cboCondicionPago.Text.Contains("CONTADO") && !cboCondicionPago.Text.Contains("PAGO MIXTO") && !cboCondicionPago.Text.Contains("CRÉDITO"))
            {
            ...
            else if (cboCondicionPago.Text.Contains("PAGO MIXTO"))
            {
                chkCobranzaMixta.Enabled = true;
                chkCobranzaMixta.Checked = true;
            ...
            else if (cboCondicionPago.Text.Contains("CRÉDITO"))
            {
                chkCobranzaMixta.Enabled = false;
                chkCobranzaMixta.Checked = false;
                gbOpcionesCobranza.Enabled = true;
        ...
        private void cboFormaPago_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (cboFormaPago.Text.Contains("DEPOSITO") || cboFormaPago.Text.Contains("IZIPAY IMAGENES")
                || cboFormaPago.Text.Contains("IZIPAY LABORATORIO") || cboFormaPago.Text.Contains("IZIPAY SAN MARCOS"))
            {
                txtNroOperacion.Enabled = true;
            }
        }
        ...
        {
            //txtTotal.Text = string.IsNullOrEmpty(txtTotal.Text.Trim()) ? "0" : txtTotal.Text.Trim();
            int moneda = cboMoneda.Value == null ? -1 : int.Parse(cboMoneda.Value.ToString());
            decimal totalVenta = !string.IsNullOrEmpty(txtTotal.Text.Trim()) ? decimal.Parse(txtTotal.Text) : 0;
            int moendaCobranza = int.Parse(cboMonedaCobranza.Value.ToString());
            decimal tipoCambio = !string.IsNullOrEmpty(txtTipoCambio.Text) ? 0 : decimal.Parse(txtTipoCambio.Text);
            txtMontoCobrar.Text = CalcularMontoCobrar(moneda, totalVenta, moendaCobranza, tipoCambio);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            frmConfigFormato frm = new frmConfigFormato();
            frm.ShowDialog();
