...
namespace SAMBHS.Windows.WinClient.UI.Mantenimientos
{
    public partial class frmBuscarProducto : Form
    {
        ...
        public string _IdProducto,_TipoTributo = "-1", _NombreProducto, _CodigoInternoProducto, _tipBusqueda, _UnidadMedida, _UnidadMedidaEmpaque,_IdProductoDetalle, _IdProductoAlmacen, _NroCuentaVenta, _NroCuentaCompra, _NroPedidoExportacion, _Observacion,_DescrProducto;
        ...
        private void BindGrid()
        {
            try
            {
                List<productoshortDto> objData = GetData("v_Descripcion ASC", _strFilterExpression);
                List<productoshortDto> objcode = objData.FindAll(p => p.v_CodInterno.Contains(_CodProducto)) == null ? null : objData.FindAll(p => p.v_CodInterno.Contains(_CodProducto));
                List<productoshortDto> objdesc = objData.FindAll(p => p.v_Descripcion.Contains(_CodProducto)) == null ? null : objData.FindAll(p => p.v_Descripcion.Contains(_CodProducto));
                if (objcode.Count > 0)
                {
                    grdData.DataSource = objcode;
                }
                else if (objdesc.Count > 0)
                {
                    grdData.DataSource = objdesc;
                }
                else
                {
        ...
        private void grdData_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            ...
            _TipoTributo = grdData.Rows[grdData.ActiveRow.Index].Cells["i_IdTipoTributo"].Value == null ? "-1" : grdData.Rows[grdData.ActiveRow.Index].Cells["i_IdTipoTributo"].Value.ToString();
