...
{
    public partial class frmCotizacionNew : Form
    {
        ...
        {
            LimpiarControles();
            AgendaBl agenda = new AgendaBl();
            var datosTrabajador = AgendaBl.GetDatosTrabajador(txtSearchNroDocument.Text);
            if (datosTrabajador != null)LlenarCampos(datosTrabajador);
            else ObtenerDatosDNI(txtSearchNroDocument.Text.Trim());
            AgendaBl.LlenarComboProtocolo_pre(cboProtocolo, 1, 2, "N009-OO000000052", "N009-OO000000052"); 
        }
        ...
        private void LlenarCampos(AgendaBl.DatosTrabajador datosTrabajador)
        {
            ...
            if (datosTrabajador.FechaNacimiento != null) dtpBirthdate.Value = datosTrabajador.FechaNacimiento.Value;
            cboDistrito.SelectedValue = datosTrabajador.Distrito;
            AgendaBl.ObtenerTodasProvincia(cboProvincia, 113);
            cboProvincia.SelectedValue = datosTrabajador.Provincia.ToString();
            AgendaBl.ObtenerTodosDepartamentos(cboDepartamento, 113);
            cboDepartamento.SelectedValue = datosTrabajador.Departamento.ToString();
        ...
        private void txtSearchNroDocument_Enter(object sender, EventArgs e)
        {
        ...
        private void frmCotizacionNew_Load(object sender, EventArgs e)
        {
            using (new LoadingClass.PleaseWait(this.Location, "Generando..."))
            {
                ...
                if (modeForm == "Edit")
                {
                    ...
                    _cotizacionDto = objCotizacion;
                    txtSearchNroDocument.Text = _docNumber;
                    btnBuscarTrabajador_Click(sender, e);
                    txtSearchNroDocument.Enabled = false;
                    cboProtocolo.SelectedValue = objCotizacion.v_ProtocolId;
                }
            };
        }
        ...
        private void btnGuardarCotizacion_Click(object sender, EventArgs e)
        {
            if (_personId == null)
            {
                MessageBox.Show("Debe guardar un paciente para continuar.", "VALIDACIÓN", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            if (cboProtocolo.SelectedValue.ToString() == "-1")
            {
                MessageBox.Show("Debe elegir un protocol para continuar.", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            ...
            {
                bool OK = false;
                using (new LoadingClass.PleaseWait(this.Location, "Creando..."))
                {

                    _cotizacionDto.v_ProtocolId = cboProtocolo.SelectedValue.ToString();
                    _cotizacionDto.d_CostoTotal = decimal.Parse(txtPrecio.Text);
                    _cotizacionDto.d_Saldo = decimal.Parse(txtPrecio.Text) - _cotizacionDto.d_aCuenta;
                    OK = CotizacionBL.UpdateCotizacion(_cotizacionDto, Globals.ClientSession.i_SystemUserId);
                }
                if (OK)
                {
                    MessageBox.Show("Se actualizó correctamente", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    this.Close();
                }
            }
            else
            {
                ...
                using (new LoadingClass.PleaseWait(this.Location, "Creando..."))
                {
                    CotizacionDto oCotizaciobDto = new CotizacionDto();
                    oCotizaciobDto.v_PersonId = _personId;
                    oCotizaciobDto.v_ProtocolId = cboProtocolo.SelectedValue.ToString();
                    //decimal CostProtocol = CotizacionBL.GetCostProtocol(oCotizaciobDto.v_ProtocolId);
                    oCotizaciobDto.d_CostoTotal = decimal.Parse(txtPrecio.Text);
                    oCotizaciobDto.d_aCuenta = 0;
                    oCotizaciobDto.d_Saldo = decimal.Parse(txtPrecio.Text) - oCotizaciobDto.d_aCuenta;
                    OK = CotizacionBL.SaveCotizacion(oCotizaciobDto, Globals.ClientSession.i_SystemUserId,
                        Globals.ClientSession.i_CurrentExecutionNodeId);
                }
                if (OK)
                {
                    MessageBox.Show("Se realizó correctamente", "AVISO", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    this.Close();
                }
            }
        ...
        private void cboProtocolo_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            {
                decimal CostProtocol = CotizacionBL.GetCostProtocol(cboProtocolo.SelectedValue.ToString());
                txtPrecio.Text = CostProtocol.ToString();
            }
        }
    }
}
