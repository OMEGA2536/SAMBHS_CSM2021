...
namespace SAMBHS.Windows.WinClient.UI.Mantenimientos
{
    public partial class frmProducto : Form
    {
        ...
        private void CargarCombos()
        {
            ...
            Utils.Windows.LoadUltraComboEditorList(cboProductSection, "Value1", "Id", _objDatahierarchyBl.GetDataHierarchiesForCombo(ref objOperationResult, 14, null), DropDownListAction.Select);
            //Utils.Windows.LoadUltraComboEditorList(cboRubroProducto, "Value1", "Id", _objDatahierarchyBl.GetDataHierarchiesForCombo(ref objOperationResult, 15, null), DropDownListAction.Select);
            //Utils.Windows.LoadUltraComboEditorList(cboClasificationProduct, "Value1", "Id", _objDatahierarchyBl.GetDataHierarchiesForCombo(ref objOperationResult, 16, null), DropDownListAction.Select);
            ...
            Utils.Windows.LoadUltraComboEditorList(cboTipoTributo, "Value1", "Id", _objDatahierarchyBl.GetDataHierarchiesForCombo_(ref objOperationResult, 35, null), DropDownListAction.Select);
        ...
        private void btnEliminarProducto_Click(object sender, EventArgs e)
        {
            ...
            if (result != DialogResult.Yes) return;
            pBuscando.Visible = true;
            List<string> v_IdMovimiento = new List<string>();
            List<string> v_IdPedido = new List<string>();
            List<string> v_IdProductoAlmacen = new List<string>();
            string pstrIdProducto = grdData.Selected.Rows[0].Cells["v_IdProducto"].Value.ToString();
            v_IdMovimiento = ObtenerListadeMov(pstrIdProducto);
            foreach (var item in v_IdMovimiento)
            {
                DeleteMovDet(item);
                DeleteMov(item);
            }

            v_IdPedido = ObtenerListaPedido(pstrIdProducto);
            foreach (var item in v_IdPedido)
            {
                DeletePedidoDet(item);
                DeleteSeparacion(item);
                DeletePedido(item);
            }

            v_IdProductoAlmacen = ObtenerListaProdAlm(pstrIdProducto);
            foreach (var item in v_IdProductoAlmacen)
            {
                DeleteProdAlm(item);
            }
            DeleteProductDet(pstrIdProducto);
            DeleteProduct(pstrIdProducto);
            pBuscando.Visible = false;
            UltraMessageBox.Show("El registro se ha eliminado correctamente", "Sistema", MessageBoxButtons.OK, MessageBoxIcon.Information);
            btnBuscar_Click(this, new EventArgs());
        }

        private void DeleteProductDet(string pstrIdProducto)
        {
            string sql = "delete from productodetalle where v_IdProducto='" + pstrIdProducto + "'";
            SQLDelete(sql);
        }

        private void DeleteSeparacion(string item)
        {
            string sql = "delete from separacionproducto where v_IdPedido='" + item + "'";
            SQLDelete(sql);
        }

        private void DeleteProduct(string pstrIdProducto)
        {
            string sql = "delete from producto where v_IdProducto='" + pstrIdProducto + "'";
            SQLDelete(sql);
        }

        private void DeleteProdAlm(string item)
        {
            string sql = "delete from productoalmacen where v_IdProductoAlmacen='" + item + "'";
            SQLDelete(sql);
        }

        private List<string> ObtenerListaProdAlm(string pstrIdProducto)
        {
            string sql = "select PA.v_IdProductoAlmacen from producto PD " +
                         "inner join productodetalle PDT on PD.v_IdProducto=PDT.v_IdProducto " +
                         "inner join productoalmacen PA on PDT.v_IdProductoDetalle=PA.v_ProductoDetalleId " +
                         "where PD.v_IdProducto='" + pstrIdProducto + "'";
            List<string> list = SQL_List(sql);
            return list;
        }

        private void DeletePedido(string item)
        {
            string sql = "delete from pedido where v_IdPedido='" + item + "'";
            SQLDelete(sql);
        }

        private void DeletePedidoDet(string item1)
        {
            string sql = "delete from pedidodetalle where v_IdPedido='" + item1 + "'";
            SQLDelete(sql);
        }

        private List<string> ObtenerPedidoDetalle(string item)
        {
            string sql = "select MV.v_IdPedidoDetalle from producto PD " +
                         "inner join productodetalle PDT on PD.v_IdProducto=PDT.v_IdProducto " +
                         "inner join pedidodetalle MV on PDT.v_IdProductoDetalle=MV.v_IdProductoDetalle " +
                         "inner join pedido MM on MM.v_IdMovimiento=MV.v_IdMovimiento " +
                         "where PD.v_IdProducto='" + item + "'";
            List<string> list = SQL_List(sql);
            return list;
        }

        private List<string> ObtenerListaPedido(string pstrIdProducto)
        {
            string sql = "select MM.v_IdPedido from producto PD " +
                         "inner join productodetalle PDT on PD.v_IdProducto=PDT.v_IdProducto " +
                         "inner join pedidodetalle MV on PDT.v_IdProductoDetalle=MV.v_IdProductoDetalle " +
                         "inner join pedido MM on MM.v_IdPedido=MV.v_IdPedido " +
                         "where PD.v_IdProducto='" + pstrIdProducto + "'";
            List<string> list = SQL_List(sql);
            return list;
        }

        private void DeleteMov(string item)
        {
            string sql = "delete from movimiento where v_IdMovimiento='" + item + "'";
            SQLDelete(sql);
        }

        private void DeleteMovDet(string item1)
        {
            string sql = "delete from movimientodetalle where v_IdMovimiento='" + item1 + "'";
            SQLDelete(sql);
        }

        private void SQLDelete(string sql)
        {
            ConexionSAM sam = new ConexionSAM();
            sam.opensam();
            SqlCommand comando = new SqlCommand(sql, sam.conectarsam);
            SqlDataReader lector = comando.ExecuteReader();
            lector.Close();
        }

        private List<string> ObtenerMovimientoDet(string item)
        {
            string sql = "select MV.v_IdMovimientoDetalle from producto PD " +
                         "inner join productodetalle PDT on PD.v_IdProducto=PDT.v_IdProducto " +
                         "inner join movimientodetalle MV on PDT.v_IdProductoDetalle=MV.v_IdProductoDetalle " +
                         "inner join movimiento MM on MM.v_IdMovimiento=MV.v_IdMovimiento " +
                         "where MM.v_IdMovimiento='" + item + "'";
            List<string> list = SQL_List(sql);
            return list;
        }

        private List<string> ObtenerListadeMov(string pstrIdProducto)
        {
            string sql = "select MM.v_IdMovimiento from producto PD " +
                         "inner join productodetalle PDT on PD.v_IdProducto=PDT.v_IdProducto " +
                         "inner join movimientodetalle MV on PDT.v_IdProductoDetalle=MV.v_IdProductoDetalle " +
                         "inner join movimiento MM on MM.v_IdMovimiento=MV.v_IdMovimiento " +
                         "where PD.v_IdProducto='" + pstrIdProducto + "'";
        ...
        private List<string> SQL_List(string sql)
        {
            ...
            while (lector.Read())
            {
                string obj = lector.GetValue(0).ToString();
                list.Add(obj);
            }
            lector.Close();
            sam.closesam();
        ...
        void CargarDatos(string pstrIdProducto)
        {
            ...
            cboProductSection.Value = (_productoDto.i_IdArte??-1).ToString();
            cboRubroProducto.Value = (_productoDto.i_IdColeccion??-1).ToString();
            cboClasificationProduct.Value = (_productoDto.i_IdTemporada??-1).ToString();
        ...
        private void btnDatosGrabar_Click(object sender, EventArgs e)
        {
            ...
            if (uvDatos.Validate(true, false).IsValid)
            {
                ...
                else if (_modeDatos == "Edit")
                {
                    ...
                    _productoDto.i_IdArte = int.Parse(cboProductSection.Value.ToString());
                    _productoDto.i_IdColeccion = int.Parse(cboRubroProducto.Value.ToString());
                    _productoDto.i_IdTemporada = cboClasificationProduct.Value ==null ?-1:  int.Parse(cboClasificationProduct.Value.ToString());
        ...
        private void LimpiarDatos()
        {
            ...
            cboProductSection.Value = "-1";
            cboRubroProducto.Value = "-1";
            txtAtributosAño.Value = Globals.ClientSession.i_Periodo.Value;
            chkDatosActivo.Checked = true;
            cboClasificationProduct.Value = "-1";
        ...
        private void btnAtributosGrabar_Click(object sender, EventArgs e)
        {
            ...
            _productoDto.i_IdArte = int.Parse(cboProductSection.Value.ToString());
            _productoDto.i_IdColeccion = int.Parse(cboRubroProducto.Value.ToString());
            _productoDto.i_IdTemporada = int.Parse(cboClasificationProduct.Value.ToString());
        ...
        private void cboProductSection_SelectionChanged(object sender, EventArgs e)
        {
            OperationResult objOperationResult = new OperationResult();
            int valueProdSection = int.Parse(this.cboProductSection.Value.ToString());

            Utils.Windows.LoadUltraComboEditorList(cboRubroProducto, "Value1", "Id", _objDatahierarchyBl.GetDataHierarchiesForComboByHeaderId(ref objOperationResult, 15, null, valueProdSection), DropDownListAction.Select);
            cboRubroProducto.SelectedItem = cboRubroProducto.Items[0];
        }

        private void cboRubroProducto_SelectionChanged(object sender, EventArgs e)
        {
            OperationResult objOperationResult = new OperationResult();
            int valueRubroProducto = int.Parse(this.cboRubroProducto.Value.ToString());

            Utils.Windows.LoadUltraComboEditorList(cboClasificationProduct, "Value1", "Id", _objDatahierarchyBl.GetDataHierarchiesForComboByHeaderId(ref objOperationResult, 16, null, valueRubroProducto), DropDownListAction.Select);
            cboClasificationProduct.SelectedItem = cboClasificationProduct.Items[0];
        }
        ...
        private void cboTipoOperacion_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
