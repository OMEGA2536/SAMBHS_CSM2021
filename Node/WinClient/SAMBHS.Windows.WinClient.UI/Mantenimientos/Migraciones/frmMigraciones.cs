...
namespace SAMBHS.Windows.WinClient.UI.Migraciones
{
    public partial class frmMigraciones : Form
    {
        ...
        private void frmMigraciones_Load(object sender, EventArgs e)
        {
            ...
            cboSeccionProducto.SelectedIndex = 0;
            cboRubroProducto.SelectedIndex = 0;
            cboClasifProducto.SelectedIndex = 0;
            cboSubFamProducto.SelectedIndex = 0;
            cboMarcaProducto.SelectedIndex = 0;
            cboUnidadxEmpaque.SelectedIndex = 0;
            cboPrecioProducto.SelectedIndex = 0;
        }
        ...
        private void ultraButton1_Click(object sender, EventArgs e)
        {
            ...
            foreach (UltraGridCell cell in grdData.Rows[0].Cells)
            {
                ...
                if (ColumnKey != "Nuevo")
                {
                    ...
                    cboSeccionProducto.Items.Add(ColumnKey);
                    cboRubroProducto.Items.Add(ColumnKey);
                    cboClasifProducto.Items.Add(ColumnKey);
                    cboSubFamProducto.Items.Add(ColumnKey);
                    cboMarcaProducto.Items.Add(ColumnKey);
                    cboUnidadxEmpaque.Items.Add(ColumnKey);
                    cboPrecioProducto.Items.Add(ColumnKey);
                }
            }
        }
            ...
            if (uvDatos.Validate(true, false).IsValid)
            {
                ...
                if (_modeDatos == "New")
                {
                    ...
                    foreach (UltraGridRow Fila in filasDepuradas)
                    {
                        ...
                        {
                            string nameLinea = cboLinea.Text == "--Seleccionar--" ? "-1" : Fila.Cells[cboLinea.Text].Value.ToString().Trim();
                            string nameSubFamilia = cboSubFamProducto.Text == "--Seleccionar--" ? "-1" : Fila.Cells[cboSubFamProducto.Text].Value.ToString().Trim();
                            string nameSeccionProd = cboSeccionProducto.Text == "--Seleccionar--" ? "-1" : Fila.Cells[cboSeccionProducto.Text].Value.ToString().Trim();
                            string nameRubroProd = cboRubroProducto.Text == "--Seleccionar--" ? "-1" : Fila.Cells[cboRubroProducto.Text].Value.ToString().Trim();
                            string nameClasifiProd = cboClasifProducto.Text == "--Seleccionar--" ? "-1" : Fila.Cells[cboClasifProducto.Text].Value.ToString().Trim();

                            string idLineaId = new ProductoBL().BuscarLineaId(nameLinea);
                            string idSubFamilia = new ProductoBL().BuscarSubFamiliaId(nameSubFamilia);
                            int idSeccion = new ProductoBL().BuscarItemIdByName(nameSeccionProd, 14);
                            int idRubro = new ProductoBL().BuscarItemIdByName(nameRubroProd, 15);
                            int idClasifi = new ProductoBL().BuscarItemIdByName(nameClasifiProd, 16);

                            decimal costo;
                            _productoDto = new productoDto();
                            _productoDto.v_CodInterno = vCodInterno;
                            _productoDto.v_IdLinea = idLineaId == null ? "-1" : idLineaId;
                            ...
                            _productoDto.i_IdArte = idSeccion;
                            _productoDto.i_IdColeccion = idRubro;
                            _productoDto.i_IdTemporada = idClasifi;
                            _productoDto.i_EsAfectoPercepcion = 0;
                            _productoDto.d_TasaPercepcion = 0;
                            _productoDto.i_Anio = 0;
                            _productoDto.v_IdTalla = idSubFamilia;
