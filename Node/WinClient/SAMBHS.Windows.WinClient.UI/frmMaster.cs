                ...
                if (objAuthorizationList.Any())
                {
                    ...
                    foreach (var menuPadre in zzz)
                    {
                        ...
                        if (!FindResult.Any()) continue;
                        {
                            ...
                            try
                            {
                                var ListaBotonesReportesFinal = new List<ButtonTool>();
                                foreach (var item in ListaBotonesReportes.ToArray())
                                {
                                    if (!ultraToolbarsManager1.Tools.Exists(item.Key))
                                    {
                                        ListaBotonesReportesFinal.Add(item);
                                    }
                                }
                                ultraToolbarsManager1.Tools.AddRange(ListaBotonesReportesFinal.ToArray());
