...
//                            
        ...
        public static float ExamenesSinFacturar(string serviceId)
        {
            using (var cnx = ConnectionHelper.GetNewSigesoftConnection)
            {
                var query = "select SUM(SC.r_Price) from servicecomponent  SC  where SC.v_ServiceId='" + serviceId +
                            "' and (SC.i_IsFact != 1 or SC.i_IsFact is null)";
            ...
            try
            {
                using (var cnx = ConnectionHelper.GetNewSigesoftConnection)
                {
                    var query = @"select SC.d_SaldoPaciente, CP.v_Name from service SR " +
                                ...
                                "where SR.v_ServiceId='" + serviceId + "' and SC.d_SaldoPaciente<>0.00 and SC.i_IsFact != 1";
