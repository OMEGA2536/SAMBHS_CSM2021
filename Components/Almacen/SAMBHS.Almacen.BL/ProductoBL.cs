...
namespace SAMBHS.Almacen.BL
{
    public class ProductoBL
    {
        ...
        public List<ProductoCustom> GetProductoCustoms(string value)
        {
            try
            {
                ...
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    var list = (from n in dbContext.producto

                                join D in dbContext.productodetalle on n.v_IdProducto equals  D.v_IdProducto

                                join E in dbContext.productoalmacen on D.v_IdProductoDetalle equals E.v_ProductoDetalleId
                                
                                join alm in dbContext.almacen on E.i_IdAlmacen equals  alm.i_IdAlmacen

                                where n.i_Eliminado == 0 && n.i_EsActivoFijo == 0 && n.i_EsActivo == 1 &&
                                      (n.v_Descripcion.Contains(value) || value == "" || n.v_IdProducto == value) && E.v_Periodo == Year
                                select new ProductoCustom
                                {
                                    v_Almacen = alm.v_Nombre,
                                    v_IdProducto = n.v_IdProducto,
                                    d_PrecioVenta = n.d_PrecioVenta,
                                    d_StockActual = E.d_StockActual,
        ...
        public string BuscarLineaId(string nameLinea)
        {
            try
            {
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    var objEntitySource = (from a in dbContext.linea
                                           where a.v_Nombre == nameLinea
                                           select a).FirstOrDefault();

                    if (objEntitySource != null)
                    {
                        return objEntitySource.v_IdLinea;
                    }
                    else
                    {
                        return null;
        ...
        public string BuscarSubFamiliaId(string nameSubfamilia)
        {
            try
            {
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    var objEntitySource = (from a in dbContext.talla
                                           where a.v_Nombre == nameSubfamilia
                                           select a).FirstOrDefault();

                    if (objEntitySource != null)
                    {
                        return objEntitySource.v_IdTalla;
                    }
                    else
                    {
                        return null;
        ...
        public int BuscarItemIdByName(string name, int groupId)
        {
            try
            {
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    var objEntitySource = (from a in dbContext.datahierarchy
                                           where a.v_Value1 == name && a.i_GroupId == groupId
                                           select a).FirstOrDefault();

                    if (objEntitySource != null)
                    {
                        return objEntitySource.i_ItemId;
                    }
                    else
                    {
                        return -1;
