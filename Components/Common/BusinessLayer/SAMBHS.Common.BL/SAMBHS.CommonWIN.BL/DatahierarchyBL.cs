...
namespace SAMBHS.CommonWIN.BL
{
    public class DatahierarchyBL
    {
        ...
        public List<KeyValueDTO> GetDataHierarchiesForCombo_(ref OperationResult pobjOperationResult, int pintGroupId, string pstrSortExpression, bool usarlAlmacenPredeterminado = false)
        {
            try
            {
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    var query = dbContext.datahierarchy.Where(a => a.i_GroupId == pintGroupId && a.i_IsDeleted == 0 && a.v_Field == "1" && a.v_Value2 != "50");
                    if (usarlAlmacenPredeterminado)
                    {
                        var idAlmacen = (Globals.ClientSession.i_IdAlmacenPredeterminado ?? 1).ToString();
                        query = query.Where(p => p.v_Value4.Trim().Equals(idAlmacen.Trim()));
        ...
        public List<KeyValueDTO> GetDataHierarchiesForComboByHeaderId(ref OperationResult pobjOperationResult, int pintGroupId, string pstrSortExpression, int pintHeaderId)
        {
            try
            {
                using (var dbContext = new SAMBHSEntitiesModelWin())
                {
                    var query = dbContext.datahierarchy.Where(a => a.i_GroupId == pintGroupId && a.i_IsDeleted == 0 && (a.i_Header == pintHeaderId || a.i_Sort == pintHeaderId));
